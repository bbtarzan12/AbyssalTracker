...
class GlobalAbyssalDataManager:
    def __init__(self, data_dir='data'):
        self.eve_api = EVEApi()
        self.data_manager = AbyssalDataManager(data_dir)
        self.console = Console() # 콘솔 출력을 위해 추가

    def _load_all_data_and_analyze(self, status_placeholder=None):
        """
        ...
        self.console.print("📂 [GlobalAbyssalDataManager] CSV 파일 로드 중...", style="bold")
        ...
        end_total = time.time()
        self.console.print(f"✨ [GlobalAbyssalDataManager] 전체 데이터 로딩 및 분석 완료. 총 소요 시간: {end_total - start_total:.2f}초 ✨", style="bold green")
    ...
    def get_all_data(self, status_placeholder=None):
        """모든 런 데이터, 통계 및 아이템 가격을 반환합니다."""
        return self._load_all_data_and_analyze(status_placeholder)

    def save_abyssal_result(self, result, start_time, end_time):
        """새로운 어비셜 런 결과를 저장합니다."""
        self.data_manager.save_abyssal_result(result, start_time, end_time)
        self.console.print("[green]✅ 새로운 런 데이터 저장 완료.[/green]")

    def clear_cache(self):
        """이 함수는 더 이상 캐시를 지우지 않습니다. 캐싱 기능이 제거되었습니다."""
        self.console.print("[green]✅ GlobalAbyssalDataManager 캐싱 기능이 제거되었습니다. 이 함수는 더 이상 동작하지 않습니다.[/green]")